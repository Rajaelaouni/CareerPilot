...
const FEATURES = [
  ...
  { icon: "🎤", title: "Entretien Vocal",  desc: "Entraînez-vous avec Whisper. Notre IA analyse votre ton et la pertinence de vos réponses.",color: C.secondary },
...
function useInView(options = {}) {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        setInView(true);
        // Ne pas déconnecter si keepAlive est demandé
        if (!options.keepAlive) obs.disconnect();
      }
        ...
        {LINKS.map(({ label, id }) => {
          ...
          return (
            <button key={id} onClick={() => scrollTo(id)} style={{
              background: isActive ? C.gradientLight : "transparent", border: "none",
    ...
    <section style={{
      ...
      <div style={{ display:"flex", gap:16, justifyContent:"center", flexWrap:"wrap", animation:"fadeInDown 0.6s 0.3s both" }}>
        <button onClick={onSignup} style={{
          ...
          borderRadius:14, cursor:"pointer", fontFamily:"'Syne',sans-serif",
        ...
        <button style={{
          ...
          color:C.text, fontSize:15, fontWeight:600, padding:"16px 32px",
...
function StatCard({ value, label, color }) {
  ...
  const numTarget = parseInt(value.replace(/\D/g, "")) || 0;
  const count     = useCounter(numTarget, inView);
...
function FeatureCard({ icon, title, desc, color, index }) {
  ...
  return (
    <div ref={ref}
      ...
      style={{
        background:C.card,
        border:`1.5px solid ${hovered ? color+"60" : C.border}`,
    ...
    <section id="features" style={{ padding:"100px 48px", background:C.bg }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        <div ref={ref} style={{
          ...
          <div style={{
            display:"inline-block", background:C.gradientLight,
            border:`1px solid ${C.border}`, borderRadius:50, padding:"6px 18px",
  ...
  return (
    <div ref={ref}
      ...
      style={{
        ...
        gap:16, position:"relative", zIndex:1, cursor:"default",
        opacity: inView ? 1 : 0,
      ...
      <div style={{
        ...
        height: hovered ? 36 : 0, overflow:"hidden",
        display:"flex", alignItems:"center", justifyContent:"center",
      }}>{step.icon}</div>
...
// ─── Testimonials ─────────────────────────────────────────
/**
 * Card témoignage avec animation scroll individuelle
 */
function TestimonialCard({ name, role, text, initiale, color, isActive, index, onClick }) {
  ...
  return (
    <div
      ...
      style={{
        cursor: "pointer",
        // Animation scroll — chaque card entre indépendamment
        opacity:    inView ? 1 : 0,
        transform:  inView
          ? isActive ? "translateY(-10px) scale(1.03)" : "translateY(0) scale(1)"
          : "translateY(50px)",
        transition: `opacity 0.6s ease ${delay}ms,
                     transform 0.5s cubic-bezier(0.34,1.3,0.64,1) ${delay}ms`,
      }}
    >
      <div style={{
        background: C.card,
        border: `1.5px solid ${isActive ? color+"50" : C.border}`,
        borderLeft: isActive ? `4px solid ${color}` : `1.5px solid ${C.border}`,
        borderRadius: 20, padding: "28px 24px",
        display: "flex", flexDirection: "column", gap: 20,
        height: "100%",
        boxShadow: isActive ? `0 20px 50px ${color}20` : "0 2px 12px rgba(0,0,0,0.04)",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:14, color:C.text, fontFamily:"'Syne',sans-serif" }}>{name}</div>
            <div style={{ fontSize:12, color:C.muted, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>{role}</div>
          </div>
        </div>
      </div>
    </div>
...
function TestimonialsSection() {
  ...
  // Carousel auto — démarre dès que la section est visible
  useEffect(() => {
    if (paused) return;
  ...
  return (
    <section id="testimonials" style={{ padding:"100px 48px", background:C.bg }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        ...
        {/* Cards — chaque card a sa propre animation scroll */}
  ...
  return (
    <section id="testimonials" style={{ padding:"100px 48px", background:C.bg }}>
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        ...
        {/* Cards avec animation scroll + carousel */}
        <div onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:24 }}>
            {TESTIMONIALS.map((t, i) => {
              const isActive = i === current;
              // Délai progressif au scroll
              const delay = i * 150;
              return (
                <div
                  key={i}
                ...
                >
                  <div style={{
                    ...
                    height: "100%",
                    boxShadow: isActive
                      ? `0 20px 50px ${t.color}20`
                      : "0 2px 12px rgba(0,0,0,0.04)",
                    transition: "all 0.4s ease",
                    // Bordure gauche colorée sur la card active
    ...
    <section style={{ padding:"80px 48px 60px", background:C.card }}>
      <div style={{
        ...
        <button onClick={onSignup} style={{
          ...
          fontSize:16, fontWeight:800, padding:"16px 40px",
          borderRadius:14, cursor:"pointer", fontFamily:"'Syne',sans-serif",
    ...
    <div style={{ background:C.bg, fontFamily:"'Plus Jakarta Sans',sans-serif", overflowX:"hidden", width:"100%", margin:0, padding:0 }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        ::-webkit-scrollbar{width:6px} ::-webkit-scrollbar-track{background:#F8F7FF} ::-webkit-scrollbar-thumb{background:#EDE8FB;border-radius:3px}
      `}</style>
